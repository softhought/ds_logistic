...
class Excavatorreport_model extends CI_Model
{
    ...
    public function getExcavatorTripReport($fromDate,$toDate,$projectid=null,$reoprtType)
    {
      $data=[];
    ...
    public function getMererialByProject($fromDate,$toDate,$projectid,$equipment_id,$reoprtType)
    {
      $data=[];
      ...
      if($query->num_rows()>0){
        foreach ($query->result() as $rows)
        {
           ...
           $data[]=[
                  ...
                  "shiftType"=>$this->getShiftType($fromDate,$toDate,$rows->material,$rows->project_material_id,$projectid,$equipment_id,$reoprtType)
                ]; 
    ...
    public function getShiftType($fromDate,$toDate,$material,$project_material_id,$projectid,$equipment_id,$reoprtType)
    {
      $data=[];
        ...
        foreach ($query->result() as $rows)
        {
             ...
             //   $data[] = $rows;
                ...
                if($reoprtType=='Quantity'){
                       ...
                       $data[]=[
                        "shiftData"=>$rows,
                        "shiftTripQuantity"=>$this->getShiftWiseQuantity($fromDate,$toDate,$material,$project_material_id,$rows->shift_code,$projectid,$equipment_id)
                      ];
                     ...
                     $data[]=[
                        "shiftData"=>$rows,
                        "shiftTripCount"=>$this->getShiftWiseTripCount($fromDate,$toDate,$material,$project_material_id,$rows->shift_code,$projectid,$equipment_id)
                      ];
...
public function getShiftWiseTripCount($fromDate,$toDate,$material,$project_material_id,$shift_code,$projectid,$equipment_id){
