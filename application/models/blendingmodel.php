...
public function getStock() {
    ...
    public function blendingRegByDate($fdate,$tdate,$cmpy,$yearId,$productid){
        if($productid>0){
             $check = array(
            "blending_master.companyid"=>$cmpy,
            "blending_master.yearid"=>$yearId,
            "blending_master.productid"=>$productid,
             );
        }else{
             $check = array(
            "blending_master.companyid"=>$cmpy,
           ...
           $this->db->select("
                          ...
                          product.`product`,
                          SUM(
                            blending_details.`number_of_blended_bag` * blending_details.`qty_of_bag`
                          ) AS blendKgs

                            ")
                         ->from('blending_master')
                         ->join('blending_details','blending_details.blending_master_id=blending_master.id','INNER')
                         ->join('product','product.id=blending_master.productid','INNER')
                         ->join('warehouse','warehouse.id=blending_master.warehouseid','INNER')
                         ->where($check)
                         ->where("blending_master.blending_date BETWEEN '".$fdate."' AND '".$tdate."' ")
                         ->group_by('blending_master.id')
                         ->order_by('blending_master.blending_date desc');
          $query = $this->db->get();   
          echo $this->db->last_query();   
           ...
           foreach ($query->result() as $rows)
            {
                $data[] = array(
                    ...
                    "blending_date" => $rows->blending_date,
                    "warehousename" => $rows->name,
                    "companyid" => $rows->companyid,
                    "yearid" => $rows->yearid,
                    "product" => $rows->product,
                    "blendKgs" => $rows->blendKgs,
                    "blendCons" => $this->getBlendQty($rows->companyid,$rows->blending_ref),
    ...
    public function getBlendQty($companyid,$blendref){
        $check = array(
            "finished_product.company_id"=>$companyid,
            "blending_master.blending_ref"=>$blendref 
        );
        ...
        $this->db->select("
                         ...
                         ->join('finished_product_dtl','finished_product_dtl.finishProductId=finished_product.id','INNER')
                         ->join('blending_master','blending_master.id=finished_product.blended_id','INNER')
                         ->join('product','product.id=finished_product.productId','INNER')
                         ->where($check);
          $query = $this->db->get();   
         # echo $this->db->last_query(); 
     ...
     public function getFiscalStartDt($yearid){
        ...
        $data = array();
        $sql="SELECT * FROM product ORDER BY product.product";
        $query = $this->db->query($sql);
            if($query->num_rows()> 0)
        {
            foreach ($query->result() as $rows)
